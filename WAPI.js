    ...
    (function () {
        ...
        function getStore(modules) {
            ...
            let neededObjects = [
                ...
                { id: "addAndSendMsgToChat", conditions: (module) => (module.addAndSendMsgToChat && module.resendMsgToChat) ? module.addAndSendMsgToChat : null },
                ...
                { id: "assertGetMeUser", conditions: (module) => (module.default && module.default.assertGetMeUser) },
                { id: "IDBFactory", conditions: (module) => (module.default && module.default.IDBFactory) },
                { id: "Broadcastmenu", conditions: (module) => (module.broadcastMenu && module.groupMenu) },
                { id: "openVoipWindow", conditions: (module) => (module.default && module.default.openVoipWindow && module.default.closeVoipWindow) },
                { id: "Toast", conditions: (module) => (module.Toast && module.ToastActionBtn) },
                { id: "setProfilePic", conditions: (module) => (module.setProfilePic && module.deleteProfilePic) },
                { id: "calculateFilteredContacts", conditions: (module) => (module.default && module.default.calculateFilteredContacts) },
                { id: "BlocklistCollection", conditions: (module) => (module.BlocklistCollection) },
                { id: "checkbroadcast", conditions: (module) => (module.addParticipants && module.removeParticipants) },
                { id: "createOrUpdateVerifiedBusinessName", conditions: (module) => (module.createOrUpdateVerifiedBusinessName && module.getAllVerifiedNames) },
                { id: "Dexie", conditions: (module) => (module.Dexie && module.default) },
                { id: "sendSetPicture", conditions: (module) => (module.sendSetPicture && module.profilePicFind) },
                { id: "abpropsConfigCreateTable", conditions: (module) => (module.abpropsConfigCreateTable && module.contactCreateTable) },
                { id: "Vcard", conditions: (module) => (module.parseVcard && module.parseMultiVcard) ? module : null },
                { id: "Features", conditions: (module) => (module.FEATURE_CHANGE_EVENT) ? module : null },
                { id: "DownloadManager", conditions: (module) => (module?.default?.downloadAndDecrypt) ? module.default : null }

            ];
            for (let idx in modules) {
                console.log(modules[idx]);
                if ((typeof modules[idx] === "object") && (modules[idx] !== null)) {
                    Object.keys(modules[idx]).forEach(x => {
                        if (typeof x == "object") {
                            Object.keys(x).forEach(y => console.log(y), modules[x]);
                        }
                        console.log(x, modules[idx]);
                    });
...
window.WAPI.SendButton = async function (ch) {
    var chat = ch.id ? ch : Store.Chat.get(ch);
    var chatId = chat.id._serialized;
    var msgIveSent = chat.msgs.filter(msg => msg.__x_isSentByMe)[0];
    if (!msgIveSent) return chat.sendMessage(body);
...
window.WAPI.sendSeen = async function (id) {
    ...
    if (chat !== undefined) {
        ...
        await Store.ReadSeen.markSeen(chat, false);
        return true;
    }
    return false;
};
...
window.WAPI.arrayBufferToBase64 = async (arrayBuffer) => {
    let binary = '';
...
window.WAPI.downloadFile2 = async function (url) {
    return await new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.onload = function () {
            if (xhr.readyState == 4) {
...
window.WAPI.DecryptFile = async function (msg) {
...
window.WAPI.sendImageId = async function (FileContent, ChatId, FileName, caption, replyId, ptt) {
    debugger;
    var ack1 = 0;
    var Id1 = '';
    var chatMsg = [];
    if (ChatId.indexOf("status@broadcast") > -1) {
        await window.WAPI.sendImage(FileContent, ChatId, FileName, caption, ptt); // <-- use it inside as you wish 
        return { Id: "", ack: 1 }
    } else {
        var contact = WAPI.getContact(ChatId);
        if (contact != null && contact != undefined) {
            await Store.Chat.find(contact.id);
            chat = WAPI.getChat(ChatId);
            await window.WAPI.sendImage(FileContent, ChatId, FileName, caption, ptt); // <-- use it inside as you wish 
            while (!chatMsg.length > 0) {
                await sleep(1000);
                chat = WAPI.getChat(ChatId);
                chatMsg = chat.msgs.models.filter(msg => msg.isSentByMe && msg.isNewMsg);
            }
            ack1 = chatMsg[chatMsg.length - 1].__x_ack;
            Id1 = chatMsg[chatMsg.length - 1].id._serialized;
            while (ack1 <= 0) {
                ack1 = window.WAPI.getMessageById(Id1).ack;
                if (ack1 <= 0) {
                    await sleep(1000);
                }
            }
            return { 'Id': Id1, 'ack': ack1 };
        }
        else {
            await window.WAPI.sendImage(FileContent, ChatId, FileName, caption, replyId, false, ptt); // <-- use it inside as you wish 
            while (!chatMsg.length > 0) {
                await sleep(1000);
                chat = WAPI.getChat(ChatId);
                chatMsg = chat.msgs.models.filter(msg => msg.isSentByMe && msg.isNewMsg);
...
window.WAPI.sendImage = function (imgBase64, chatid, filename, caption, ptt) {
...
window.WAPI.sendMessageWithUnknown = function (id, body) {
    var OriginalId;
    var Chat = window.WAPI.getChat(id);
    if (Chat == null) {
        Chat = Store.Chat.models[0];
        var OriginalId = Chat.id;
        Chat.id = typeof OriginalId === "string" ? id : new window.Store.UserConstructor(id, { intentionallyUsePrivateConstructor: true });
    }
    var Return = window.WAPI.sendMessageReturnId(Chat, body);
    if (OriginalId) {
        Chat.id = OriginalId;
    }
    return Return;
...
//             await sleep(1000);
//             chat = WAPI.getChat(ChatId);
//             chatMsg = chat.msgs.filter(msg => msg.__x_isSentByMe && msg.__x_isNewMsg);
//         }
//         ack1 = chatMsg[chatMsg.length - 1].__x_ack;
//         Id1 = chatMsg[chatMsg.length - 1].id._serialized;
//         while (ack1 <= 0) {
...
//     if (chatid.includes('@g')) return false;
//     let extras = {};
//     if (quotedMsg && quotedMsg != "0") {
...
window.WAPI.base64ImageToFile = function (b64Data, filename) {
    ...
    var FileData = new File([u8arr], filename, { type: mime })
    u8arr = null;
    return FileData;
};
...
window.WAPI.sendLocation = async (chatId, content, options = {}) => {
    debugger;
    var chat = Store.Chat.get(chatId);
    let locationOptions = {};
    if (options.location) {
    ...
    const newMsgId = new window.Store.MsgKey({
        fromMe: true,
        remote: chat.id,
        id: window.WAPI.getNewMessageId(chatId),
    });
...
window.WAPI.sendVCard = async (chatId, content, options = { parseVCards: true }) => {
    debugger;
    var chat = Store.Chat.get(chatId);
    ...
    let vcardOptions = {};
    if (options.parseVCards && typeof (content) === 'string' && content.startsWith('BEGIN:VCARD')) {
...
//     var tempMsg = Object.create(Store.Msg.models.filter(msg => msg.__x_isSentByMe && !msg.quotedMsg)[0]);
//     var newId = window.WAPI.getNewMessageId(chatId);
//     var parsed = window.Store.Vcard.parseVcard(vcard);
//     var extend = {
//         id: newId,
//         ack: 0,
//         body: vcard,
...
window.WAPI.reply = async function (chatId, body, quotedMsg) {
    ...
    var chat = Store.Chat.get(chatId);
    if (!chat) return false;
    var res = await chat.sendMessage(body, {
        linkPreview: null,
...
window.WAPI._sendSticker = async function (sticker, chatId, metadata) {
    ...
    stick.encFilehash = sticker.encFilehash;
...
window.WAPI.encryptAndUploadFile = async function (type, blob) {
    let filehash = await Store.CryptoLib.getHash(blob);
